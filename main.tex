...
\section*{Rešenje (Solution) i projekat (Project)}
\textbf{Projekat} je jedna aplikacija ili biblioteka koja se kompajlira. U .NET svetu projekat je definisan fajlom \texttt{*.csproj}. U njemu se nalaze: target framework, reference na NuGet pakete, reference na druge projekte, opcije build-a, itd.

\textbf{Rešenje} je kontejner koji organizuje više projekata u jednu celinu. Tipično je definisano fajlom \texttt{*.sln}. Rešenje ne sadrži kod koji se izvršava; služi za organizaciju u IDE-u (npr. jedan solution može sadržati više projekata: aplikacija + biblioteka + testovi).

\textbf{Važno:} možete pokretati projekat i bez \texttt{*.sln} fajla. Komande kao što su \texttt{dotnet build} i \texttt{dotnet run} rade nad \texttt{*.csproj} (ili nad folderom koji ga sadrži).

U ovom materijalu svaki mini-projekat je jedan zaseban folder sa jednim Avalonia projektom. Rider može da otvori folder projekta direktno (File/Open), i u tom slučaju će napraviti interni "solution model" bez potrebe da ručno kreirate \texttt{*.sln}.
...
\textbf{Kada:} pre nego što krenete sa ovom sekcijom.

\textbf{Kako (preporučeno):} kreirajte projekat iz terminala, pa otvorite folder u Rider-u.
...
\textbf{Kada:} pre nego što krenete sa sekcijama \textit{Code-behind} i \textit{Događaji}.

\textbf{Kako (preporučeno):}
