...
% Phase 1 migration: structure + WPF text transcribed into LaTeX.
% NOTE: This document intentionally preserves WPF wording for now.
% Next phases will rewrite sections to Avalonia + Rider.
...
\newcommand{\MigrationNote}[1]{%
  \begin{quote}
    \color{black!75}\textbf{Migration note (Phase 1):} #1
  \end{quote}
}
...
\section*{Migration Checklist (Phase 1)}
\MigrationNote{This is an extraction of the existing WPF PDF into LaTeX. Content is preserved; wording is not yet adapted to Avalonia + Rider. Figures are placeholders (original screenshots are not imported yet).}

\begin{itemize}[leftmargin=*,nosep]
  \item \textbf{Keep (concepts):} markup vs code-behind, events, layout panels, data binding, resources, converters.
  \item \textbf{Rewrite (tooling):} project creation steps, IDE workflow, designer/toolbox references, screenshots.
  \item \textbf{Replace (APIs/examples):} WPF-specific namespaces/types, MessageBox examples, WPF-specific validation/adorners, WPF documentation links.
  \item \textbf{Re-shoot:} all Visual Studio screenshots (Rider cross-platform).
\end{itemize}
...
\section{Uvod u WPF}
\MigrationNote{Rewrite section as \emph{Uvod u Avalonia}. Keep GUI/XAML framing, replace WPF-specific statements with Avalonia equivalents and cross-platform positioning.}
...
\subsection{Kreiranje WPF projekta}
\MigrationNote{Replace with Rider + \texttt{dotnet new} Avalonia templates instructions. Replace all screenshots.}

Jedan od načina kreiranja novog WPF projekta je odabirom File/New/Project.

\FigurePlaceholder{slika-1.png}{Slika 1. Kreiranje novog projekta}

Potrebno je odabrati WPF App (.NET Framework) i popuniti Name i Location polja.

\FigurePlaceholder{slika-2.png}{Slika 2. Odabir WPF projekta}

\subsection{Markup i code-behind}
\MigrationNote{Keep concept, update IDE navigation screenshots and Avalonia project file names.}
...
\begin{enumerate}[leftmargin=*]
  ...
  \item Globalizacija i lokalizacija za WPF aplikacije je pojednostavljena.
\end{enumerate}

Na slici se može videti otvoren prazan WPF projekat i brojevi kojima su označeni najbitniji segmenti za razvoj WPF aplikacije.
...
Na slici možemo primetiti da je otvoren MainWindow.xaml fajl što zapravo predstavlja XAML kod ovog projekta.

\FigurePlaceholder{slika-3.png}{Slika 3. Okruženje i XAML kod}

Ako se u Solution Exploreru (sekcija 4) otvori MainWindow.xaml.cs dobija se code behind.

\FigurePlaceholder{slika-4.png}{Slika 4. Code behind}

Osim odabirom iz Solution Explorera, jednostavan način da se iz XAML koda pređe na .cs fajl je desni klik i odabir View Code opcije iz padajućeg menija.

\FigurePlaceholder{slika-5.png}{Slika 5. Prelazak sa XAML koda na Code behind}

Prelazak iz .cs fajla na dizajner (XAML) moguć je pomoću desni klik pa na View Designer opciju.

\FigurePlaceholder{slika-6.png}{Slika 6. Prelazak sa Code behind na XAML kod}
